...
**Objective:** Learn how to teach an AI about your own private documents entirely locally. You will build a system that reads an uploaded file, converts the text into a mathematical format the AI can understand using local models, and allows users to chat with the document using a local AI Agent.

**Prerequisites:**

* Access to your n8n environment.
* A local instance of Ollama running with your preferred models pulled (e.g., `qwen3:8b` for chat and `qwen3-embedding` for embeddings).
* A sample document (like a PDF syllabus or a research paper on high-performance computing).
...
1. Add an **Embeddings Ollama** node to your canvas.
2. Configure it to connect to your local Ollama instance (usually `http://localhost:11434` or your server's IP).
3. Select your embedding model (e.g., `qwen3-embedding`).
4. *What it does:* This node takes the text chunks from the Data Loader and converts them into vectors (those lists of numbers representing semantic meaning) completely locally, keeping your data private.

### Step 4: Insert Data to Store (n8n Vector Store)

1. Add an **n8n Vector Store** node.
...
* Connect the **Embeddings Ollama** node to the `Embedding` input (the math).


4. *What it does:* This securely stores your text and its mathematical representation directly within n8n's internal database, eliminating the need to connect to external cloud vector databases.
...
2. Attach an **Ollama Chat Model** node to the `Model` input of the Agent.
3. Configure it to connect to your local Ollama instance and select a capable chat model (e.g., `qwen3:8b`).
4. *What it does:* The Agent acts as the brain. It receives the user's chat message and decides what to do next.
...
1. Add another **n8n Vector Store** node.
...
5. **The Golden Rule of Embeddings:** You must connect the exact same **Embeddings Ollama** node from Part 1 into the `Embedding` input of this Query node.
* *Why?* If the text was translated into math using a specific local model, the search query must also be translated into math using that exact same model. If you use different translators, the math won't match, and the search will fail!
...
2. First, click the URL provided by the **Form Trigger** to open the web form. Upload your PDF.
3. Wait a few seconds for the data to process through the Load pipeline using your local GPU resources.
...
6. Watch the AI Agent call the `knowledge_base` tool, retrieve the mathematical matches from the local datastore, and generate a perfect, document-specific answer without any data ever leaving your machine!
